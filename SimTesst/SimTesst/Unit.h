...
enum class ActionState : uint8_t { Idle, Moving, AttackWindup, AttackRecover, Casting, Defending, Focusing, Stunned, Dead };
...
public:
	...
	float  hp = 100.f;
	float  maxHp = 100.f;                 // 최대 체력 (클래스 주입, 힐 회복 상한)
	bool   alive = true;
	...
	bool defendStance = false;                 // 방어 태세 중(방어 스킬 시전 중)
